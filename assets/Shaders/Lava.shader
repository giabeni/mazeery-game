...
float rand(vec2 coord){
	return fract(sin(dot(coord, vec2(56.0934483, 78.3674596)) * 3000.0) * 4000.0);
}
...
void fragment() {
	...
	if (combined > 0.25) {
		ALBEDO = mix(red.rgb, yellow.rgb, (combined - 1.0) * 1.0 + 1.0);
		EMISSION = vec3(ALBEDO)
	} else {
		ALBEDO = mix(grey.rgb, black.rgb, combined * 3.0);
	}
}
